...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── Booking Panel (additive styles only — nothing above is modified) ── */
    .booking-section { margin-top: 20px; }
    .booking-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
    .book-input { width: 100%; background: #0d1117; color: var(--park-text); border: 1px solid var(--park-border); border-radius: 6px; padding: 8px 10px; font-size: 13px; outline: none; margin-bottom: 8px; }
    .book-input:focus { border-color: var(--park-blue); }
    .btn-book { background: rgba(46,160,67,0.15); color: var(--park-green); border-color: rgba(46,160,67,0.4); width: 100%; justify-content: center; }
    .btn-book:hover { background: rgba(46,160,67,0.25); }
    .btn-cancel-booking { background: rgba(218,54,51,0.12); color: var(--park-red); border-color: rgba(218,54,51,0.3); font-size: 11px; padding: 4px 10px; }
    .btn-cancel-booking:hover { background: rgba(218,54,51,0.22); }
    .book-msg { font-size: 12px; margin-top: 8px; padding: 7px 10px; border-radius: 6px; display: none; }
    .book-msg.success { background: rgba(46,160,67,0.15); color: var(--park-green); border: 1px solid rgba(46,160,67,0.3); display: block; }
    .book-msg.error   { background: rgba(218,54,51,0.15); color: var(--park-red);   border: 1px solid rgba(218,54,51,0.3);   display: block; }
    .bookings-table { width: 100%; border-collapse: collapse; font-size: 12px; }
    .bookings-table th { text-align: left; color: var(--park-muted); font-weight: 500; padding: 6px 8px; border-bottom: 1px solid var(--park-border); font-size: 11px; text-transform: uppercase; letter-spacing: 0.04em; }
    .bookings-table td { padding: 7px 8px; border-bottom: 1px solid rgba(255,255,255,0.04); vertical-align: middle; }
    .bookings-table tr:last-child td { border-bottom: none; }
    .badge-slot { background: rgba(218,54,51,0.15); color: var(--park-red); border: 1px solid rgba(218,54,51,0.3); padding: 2px 8px; border-radius: 4px; font-weight: 600; font-size: 11px; }
    .badge-ev { background: rgba(63,185,80,0.15); color: var(--park-ev); border: 1px solid rgba(63,185,80,0.3); padding: 2px 6px; border-radius: 4px; font-size: 10px; margin-left: 4px; }
    .no-bookings { text-align: center; color: var(--park-muted); font-size: 12px; padding: 20px 0; }
...
<div id="psd">
  ...
  <div class="insights-row">
    ...
    <div class="insight-card">
      ...
      <div class="insight-val" style="color:var(--park-blue)">1–2h</div>
  ...
  <!-- ── BOOKING SECTION (additive — zero changes above this line) ───────── -->
  <div class="booking-section">
    <div class="booking-grid">

      <div class="panel">
        <div class="panel-hdr">
          <span class="panel-title">&#128221; Book a Slot</span>
          <span style="font-size:11px;color:var(--park-muted)">Reserve ahead</span>
        </div>
        <div class="panel-body">
          <input  class="book-input" id="book-user"        type="text" placeholder="Your name" autocomplete="off"/>
          <select class="book-input" id="book-slot-select" style="cursor:pointer">
            <option value="">-- Select available slot --</option>
          </select>
          <button class="btn btn-book" onclick="submitBooking()">&#10003; Confirm Booking</button>
          <div class="book-msg" id="book-msg"></div>
        </div>
      </div>

      <div class="panel">
        <div class="panel-hdr">
          <span class="panel-title">&#128203; Current Bookings</span>
          <span id="booking-count" style="font-size:11px;color:var(--park-muted)">0 active</span>
        </div>
        <div class="panel-body" style="padding:0">
          <div style="overflow-x:auto;padding:0 4px">
            <table class="bookings-table">
              <thead>
                <tr><th>Slot</th><th>User</th><th>Time</th><th></th></tr>
              </thead>
              <tbody id="bookings-tbody">
                <tr><td colspan="4" class="no-bookings">No bookings yet</td></tr>
              </tbody>
            </table>
          </div>
        </div>
...
<script>
  // ═══════════════════════════════════════════════════════════════════════════
  // ORIGINAL CORE LOGIC — NOT MODIFIED
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  function updateMetrics() {
    ...
    updateDropdown();
    updateHeatmap();
    updateBookingDropdown(); // keep booking form in sync
  }
  ...
  // refreshData — only change: also re-syncs bookings after simulation tick
  function refreshData(fromScan = false) {
    initOccupied();
    syncBookingsIntoOccupied().then(() => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // END ORIGINAL CORE LOGIC
  // ═══════════════════════════════════════════════════════════════════════════


  // ═══════════════════════════════════════════════════════════════════════════
  // BOOKING MODULE — separate concern, talks to /book /bookings /cancel APIs
  // ═══════════════════════════════════════════════════════════════════════════
  let bookedSlots = new Set();

  async function syncBookingsIntoOccupied() {
    try {
      const res  = await fetch('/bookings');
      const data = await res.json();
      bookedSlots.clear();
      data.bookings.forEach(b => {
        bookedSlots.add(b.slot);
        occupied.add(b.slot);   // merge: booked slot = visually occupied (red)
      });
      renderBookingsTable(data.bookings);
      document.getElementById('booking-count').textContent = `${data.count} active`;
    } catch (e) {
      console.warn('Could not sync bookings:', e);
    }
  }
  ...
  function renderBookingsTable(bookings) {
    ...
    if (!bookings.length) {
      tbody.innerHTML = '<tr><td colspan="4" class="no-bookings">No bookings yet</td></tr>';
      return;
    }
    tbody.innerHTML = bookings.map(b => `
      <tr>
        <td>
          <span class="badge-slot">${slotId(b.slot)}</span>
          ${EV_SLOTS.has(b.slot) ? '<span class="badge-ev">EV</span>' : ''}
        </td>
        <td style="color:var(--park-text)">${escHtml(b.user)}</td>
        <td style="color:var(--park-muted);font-family:monospace">${b.time}</td>
        <td>
          <button class="btn btn-cancel-booking" onclick="cancelBooking(${b.slot})">Cancel</button>
        </td>
      </tr>`).join('');
  }

  function updateBookingDropdown() {
    const sel = document.getElementById('book-slot-select');
    const cur = sel.value;
  ...
  async function submitBooking() {
    const user = document.getElementById('book-user').value.trim();
    const slot = parseInt(document.getElementById('book-slot-select').value);
    if (!user) { showBookMsg('Please enter your name.', false); return; }
    if (!slot) { showBookMsg('Please select a slot.', false);  return; }
    try {
      ...
      if (res.ok) {
        ...
        document.getElementById('book-slot-select').value = '';
        addFeedEntry(`Slot ${slotId(slot)} booked by ${user}`, '#2ea043');
        await syncBookingsIntoOccupied();
        buildGrid();
        updateMetrics();
      } else {
        showBookMsg(`&#10007; ${data.error}`, false);
      }
    } catch (e) {
      showBookMsg('Network error. Is Flask running?', false);
    }
  }
  ...
  async function cancelBooking(slot) {
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        addFeedEntry(`Booking for ${slotId(slot)} cancelled`, '#e3b341');
        initOccupied();                        // re-run simulation
        await syncBookingsIntoOccupied();      // re-merge remaining bookings
        buildGrid();
        updateMetrics();
      } else {
        addFeedEntry(`Cancel failed: ${data.error}`, '#da3633');
      }
    } catch (e) { console.warn('Cancel error:', e); }
  }

  function showBookMsg(text, success) {
    const el = document.getElementById('book-msg');
    el.innerHTML = text;
    el.className = 'book-msg ' + (success ? 'success' : 'error');
    setTimeout(() => { el.className = 'book-msg'; }, 4000);
  }
  ...
  // Boot
  initOccupied();
  syncBookingsIntoOccupied().then(() => { buildGrid(); updateMetrics(); });
...
if __name__ == "__main__":
    ...
    print("  + Slot Booking Module (Blueprint)")
    print("=" * 50)
    print("  http://127.0.0.1:5000        → Dashboard")
    print("  POST /book                   → Book a slot")
    print("  GET  /bookings               → All bookings")
    print("  POST /cancel                 → Cancel booking")
    print("=" * 50)
